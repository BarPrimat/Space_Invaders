...
namespace GameSprites
{
    public class EnemyArmy : GameComponent
    {
        ...
        private void setupNewArmy()
        {
            ...
            m_EnemyNextTimeToShoot = r_Random.Next((int) StartEnemyMinTimeForShoot, (int) EnemyMaxTimeForShoot);
        ...
        private void resetEnemyForNewGame(int i_Row, int i_Column)
        {
            if (i_Column > r_EnemyArray[i_Row].Count - 1)
            {
                initEnemyArmy(i_Row, i_Column);
            }
            else
            {
                r_EnemyArray[i_Row][i_Column].BackToLifeIfNeeded();
            }
        }
        ...
        private void OneEnemyIsDead_EnemyIsKilled()
        {
            m_EnemyThatLeft--;
